...
def shorten_path_end(path_str: str, max_len: int) -> str:
    if max_len <= 0:
        return ""
    if not path_str:
        return ""
...
class GrabberDaemon:
    ...
    def _render(self, first: bool = False):
        try:
            ...
            print(self._box_line(f"{col_jid} {col_id} {col_total} STATUS/PROGRESS", w))
            print(self._box_border(w))
            ...
            if elap_col_start is None:
                elap_col_start = min(text_area - 12, 72)
            ...
            def render_job(j: Job):
                ...
                if j.status == "fail":
                    ...
                    status = c("[FAIL]", RED)
                    content = f"{jid_col} {id_col} {total_col} {status} {err}"
                    print(self._box_line(content, w))
                    return
